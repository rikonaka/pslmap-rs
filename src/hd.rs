...
pub fn host_discovery(
    ...
    match hd_method {
        HostDiscoveryMethod::Mac => host_discovery_by_mac(targets, log_level, timeout),
        _ => host_discovery_by_ping(targets, hd_method, log_level, timeout),
    }
}
...
fn host_discovery_by_mac(targets: &[Target], log_level: PistolLogger, timeout: f64) {
    let start = Instant::now();
